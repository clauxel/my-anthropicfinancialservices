...
const LIVE_ORIGIN = 'https://anthropicfinancial.space'
const LIVE_HOST = 'anthropicfinancial.space'
const ALT_HOSTS = new Set(['www.anthropicfinancial.space'])
...
const planCatalog = {
  analyst: {
    ...
    currency: 'USD',
    summary: 'one supervised Claude financial-services workflow',
  },
  desk: {
    id: 'desk',
    name: 'Desk',
    ...
    currency: 'USD',
    summary: 'default managed finance-agent pilot workspace',
  },
  firm: {
    id: 'firm',
    name: 'Firm',
    ...
    currency: 'USD',
    summary: 'internal systems and firm-control rollout planning',
  },
}

const indexablePaths = [
  '/',
  '/anthropic-financial-services-github-project-explained',
  '/anthropic-financial-services-github',
  '/anthropic-ai-agents-financial-services',
  '/anthropic-financial-services-plugin',
  '/anthropic-financial-services-event',
  '/anthropic-financial-services-jobs',
  '/claude-for-financial-services-demo',
  '/claude-for-financial-services-review',
  '/anthropic-claude-financial',
  '/pricing',
  '/privacy',
  '/terms',
]
...
function resolveConfiguredProductId(env, planId, billing) {
  ...
  const tier = planId === 'firm' ? 'FIRM' : planId === 'analyst' ? 'ANALYST' : 'DESK'
  const normalizedSelection = normalizeEnvKey(`${planId}_${billing}`)
  const keys = [
    `CREEM_PRODUCT_${tier}_${cycle}`,
    `CREEM_PRODUCT_ID_ANTHROPIC_FINANCIAL_${normalizedSelection}`,
...
async function getOrCreateCreemProduct(env, apiKey, plan, billing, successUrl) {
  ...
  const product = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/products`, {
    name: `Anthropic Financial Services ${plan.name} (${billingLabel})`,
...
async function handleCheckout(request, env, requestUrl) {
  ...
  const planId = typeof body?.planId === 'string' ? body.planId : 'desk'
  const billing = body?.billing === 'monthly' ? 'monthly' : 'annual'
  const plan = planCatalog[planId] || planCatalog.desk
  ...
  try {
    ...
    const checkout = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/checkouts`, {
      ...
      request_id: `anthropic_financial_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      metadata: {
        site: 'anthropicfinancial.space',
        planId: plan.id,
        billing,
      },
    })
...
function handleCatalog(request) {
  return jsonResponse(
    {
      ok: true,
      defaultPlan: 'desk',
      ...
      plans: Object.values(planCatalog).map((plan) => ({
        ...
        annualMonthlyEquivalentCents: Math.round(plan.monthlyAmountCents * ANNUAL_DISCOUNT_MULTIPLIER),
        currency: plan.currency,
      })),
    },
    200,
...
async function handleAnalytics(request, env) {
  return handleAnalyticsRequest(request, env, { siteKey: 'anthropicfinancialservices' })
}
...
export async function handleRequest(request, env) {
  ...
  if (requestUrl.pathname === '/api/nowpayments-checkout') {
    return handleNowPaymentsCheckout(request, env, {
      plans: planCatalog,
      defaultPlanId: 'analyst',
      siteName: 'anthropicfinancialservices',
      siteKey: 'anthropicfinancialservices',
  ...
  if (requestUrl.pathname === '/api/catalog') return handleCatalog(request)
