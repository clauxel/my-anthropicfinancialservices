...
const liveOrigin = 'https://anthropicfinancial.space'
...
const canonicalLinkId = 'afs-canonical-link'
const structuredDataScriptId = 'afs-structured-data'
...
export function buildSeoDocument(args: {
  ...
  if (routeView === 'pricing') {
    const title = 'Anthropic Financial Services Pricing | Desk Annual Plan'
    const description =
  ...
  if (routeView === 'privacy') {
    ...
    const description = 'How this site handles analytics, checkout metadata, support requests, and hosted payment interactions.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  if (routeView === 'terms') {
    ...
    const description = 'Terms for using this supervised financial-services agent workspace, pricing flow, and hosted checkout.'
    return {
      title,
      description,
      canonicalUrl,
